...
halves_scored_allowed |>
  mutate(h1_margin = half_scored - half_allowed,
         h2_margin = second_half_scored - second_half_allowed,
         abs_diff = abs(h1_margin - h2_margin),
         class = case_when(h1_margin > 0 & h2_margin > 0 ~ "always good",
                           h1_margin > 0 & h2_margin <= 0 ~ "first half team",
                           h1_margin <= 0 & h2_margin > 0 ~ "second half team",
                           h1_margin <= 0 & h2_margin <= 0 ~ "always bad")) |>
  select(team, class)
```
